...
<Link to="/" className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center bg-[#2c2c28] text-lg font-semibold tracking-tight text-white">SP</div><div className="hidden sm:block leading-tight"><div className="text-sm font-semibold tracking-[-.02em]">Shapoorji Pallonji</div><div className="text-[10px] uppercase tracking-[.17em] text-[#7a7b74]">Real Estate</div></div></Link>
...
<div className="hidden lg:flex items-center gap-4"><a href="tel:+918700983465" className="text-xs font-medium text-[#686a63] hover:text-[#20221f]">+91 87009 83465</a><button onClick={()=>onOpenEnquiryModal?.('Residential Enquiry')} className="inline-flex items-center gap-2 bg-[#262723] px-4 py-2.5 text-xs font-semibold text-white transition hover:bg-[#3a3b34]">Book a conversation<ArrowUpRight className="h-3.5 w-3.5"/></button></div>
<div className="flex items-center gap-2 lg:hidden"><button onClick={()=>onOpenEnquiryModal?.('Residential Enquiry')} className="bg-[#262723] px-3 py-2 text-xs font-semibold text-white">Enquire</button><button onClick={()=>setOpen(!open)} className="p-2 text-[#262723]" aria-label="Toggle navigation">{open?<X className="h-6 w-6"/>:<Menu className="h-6 w-6"/>}</button></div>
</div>
